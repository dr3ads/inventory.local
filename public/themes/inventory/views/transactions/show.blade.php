        ...
        <div class="transaction-details">
            <div class="controls">
                ...
                @if($transaction->status == 'active')
                    <a class="btn btn-default pull-left margin-left-20 link-tooltip @if( $transactionDetails['totalPawnAmount'] >= $transaction->item->value  ) disabled @endif"
                       data-placement="bottom" data-toggle="tooltip" title="Repawn Transaction"
                       href="{{ url('transactions/repawn/'.$transaction->id) }}" id="repawn_link"
                       title="Repawn">
                ...
                @if($transaction->status != 'claimed')
                    <a class="btn btn-default pull-left link-tooltip" data-placement="bottom" data-toggle="tooltip"
                       title="Claim Item" href="{{ url('transactions/claim/'.$transaction->id) }}"
                       id="claim_link" title="Claim">
                        ...
                        <a class="btn btn-info pull-left link-tooltip" data-placement="bottom" data-toggle="tooltip"
                           title="Renew Transaction" href="{{ url('transactions/renew/'.$transaction->id) }}"
                ...
                @if($transaction->status == 'expired')
                    <a class="btn btn-info pull-left link-tooltip" data-placement="bottom" data-toggle="tooltip"
                       title="Hold Transaction" href="{{ url('transactions/hold/'.$transaction->id) }}"
                ...
                @if($transaction->status == 'void')
                    <a class="btn btn-info pull-left link-tooltip" data-placement="bottom" data-toggle="tooltip"
                       title="Hold Transaction" href="{{ url('transactions/display/'.$transaction->id) }}"
                       id="hold_link" title="Hold">
